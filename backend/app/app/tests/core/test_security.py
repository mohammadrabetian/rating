...
def test_security_not_authorized(client: TestClient, rate_cdr_obj: dict):
    r = client.post(f"{settings.API_V1_STR}/rate/", data=rate_cdr_obj)
    assert r.status_code == 403
    assert r.json() == {"detail": "Not authenticated"}


def test_security_wrong_api_key(client: TestClient, rate_cdr_obj: dict):
...
def test_security_correct_api_key(client: TestClient, rate_cdr_obj: dict):
